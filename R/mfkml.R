...
#           the input must include a Weight column with the specified values.
#
# - seed: A numeric value used to set the random seed for reproducibility.
#         Controls the order in which pairwise Fréchet means are computed and averaged.
#
# Output:
...
#' 
#' @details
#' The input dataset (\code{dt}) must contain only numeric values (except for the ID column) and must not include any missing values. 
#' Each variable should be measured at least three times per trajectory, as the analysis is shape-based. 
#' Two observations are insufficient to capture shape trends such as increasing, decreasing, or stable patterns.
#' 
#' Since the Fréchet distance is sensitive to measurement units, proper scaling is essential when applying the \code{mfkml} function. 
#' The \code{scales} vector, which contains scaling factors for time and each variable, is used to rescale the corresponding columns. 
#' This prevents distortion due to differences in the scale of time and variables, enabling more accurate shape-based comparisons.
#'
...
mfkml <- function(dt, clt_n, scales, weight, seed, maxIter = 50) {
  ...
  while (!identical(group, exGroup) && iter < maxIter) {
    ...
    fMean <- function(cent_i) {
      data.frame(Cluster = cent_i,
                 fremean(dt[groupl == cent_i, ], seed = seed))
    }
