...
UpdateCs.SFclust <- function(k, dist.ary, wss = NULL, seed) {
...
SFclust <- function(k, seed, l1bound, dist.ary, maxIter = 20, eps = 1e-4) {
  ...
  css <- UpdateCs.SFclust(k, dist.ary, seed = seed)
  ...
  while (count < maxIter & (sum(abs(wss-wss.old)^2)/(num.var*sum(abs(wss.old)))) > eps) {
    ...
    css <- UpdateCs.SFclust(k, dist.ary, wss, seed)
...
SFclust.permute <- function(dist.ary, k, nperms, seed, l1b,  plot.gap = TRUE, plot.gap.l1b = TRUE) {
  ...
  for (j in 1:length(l1b)) {
    out <- tryCatch(
      SFclust(k, seed, l1b[j], dist.ary = dist.ary),
      error = function(e) {
  ...
  for (kk in 1:nperms) {
    permx[[kk]] <- permute.distance.array(dist.ary, seed = seed.vec[kk])
  }
  ...
  for (kk in 1:nperms) {
    ...
    for (j in setdiff(1:length(l1b), failed_j)) {
      perm.out <- tryCatch(
        SFclust(k, seed, l1b[j], dist.ary = dist.ary.pm),
        error = function(e) {
  ...
  if (plot.gap) {
    if (plot.gap.l1b) {
      p <- ggplot(data = data.frame(x = l1b[setdiff(1:length(l1b), failed_j)], y = gaps), aes(x = x, y = y)) + 
        geom_line() + 
        geom_point() +
        labs(x = "l1bound (s)", y = "Gap", title = paste("k = ", k)) +
        theme_minimal()
    } else {
      p <- ggplot(data = data.frame(x = nnonzerowss, y = gaps), aes(x = x, y = y)) + 
        geom_point() + 
  ...
  return(list(totss = totss, permtotss = permtotss, nnonzerowss = nnonzerowss, 
